...
public class EntrySummary extends AbstractModel<EntrySummary> {
	...
	private EntrySummary(String id, String title, Category category,
			String sponsor, String schedule) {
		super(id);
		this.title = title;
	...
	protected void init(SQLiteDatabase database) {
		...
		for (int i = 0, rows = c1.getCount(); i < rows; i++) {
			...
			EntrySummary es = new EntrySummary(id, title, category, sponsor,
					schedule);
			factory.put(id, es);
			c1.moveToNext();
		}
		...
		Cursor c2 = database.query(table2, columns2, null, null, null, null,
				null);
	...
	private CharSequence enhanceSchedule(CharSequence schedule) {
		...
		for (Day day : Day.values()) {
			...
			while (p > -1) {
				...
				ss.setSpan(android.graphics.Typeface.BOLD, 0, ss.length(),
						Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
				ss.setSpan(new ForegroundColorSpan(day.getColor()), 0,
